...
def Condition(df, symbol):
    ...
    backCandles = 10 #45 or 90 or 180 use for checking backward if the structure occurred thus the higher the backcandle the number of levels
    ...
    for row in range(backCandles, len(df)-no_after):
        ...
        # Checking to see if structure is too close
    ...
    for current_candle in range(len(df)-lookback, len(df)):
        ...
        elif (DarkLine(current_candle, close, open_, body_diff) == 2 and closeSupport(current_candle, sup_lvl, 0.9, df) == 2):
            if (df['Momentum'][current_candle] > 0 and df['MACD'][current_candle] > df['MACDsignal'][current_candle] or df['MACDsignal'][current_candle] > 0 or df['RSI'][current_candle] < 50 and df['RSI'][current_candle] > 20 ): 
                signal[current_candle] = 2
